/**
 * Visual cues associated with Google reviews (multicolor G mark + star row).
 * Replace outbound links with your real Business Profile / review URLs in page data.
 */

function GoogleGMark({ className }: { className?: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 24 24"
      width="24"
      height="24"
      aria-hidden
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  );
}

function GoogleStyleStar({ className }: { className?: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 24 24"
      width="24"
      height="24"
      aria-hidden
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        fill="#FBBC04"
        d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"
      />
    </svg>
  );
}

type GoogleReviewBadgeProps = {
  className?: string;
  /** Show numeric rating like Google (e.g. 5.0) */
  showScore?: boolean;
  score?: string;
  starClassName?: string;
};

export function GoogleReviewBadge({
  className = "",
  showScore = true,
  score = "5.0",
  starClassName = "h-[18px] w-[18px]",
}: GoogleReviewBadgeProps) {
  return (
    <div className={`flex flex-wrap items-center gap-2 ${className}`}>
      <span className="inline-flex items-center gap-2 rounded-lg border border-navy/10 bg-white px-2 py-1 shadow-sm">
        <GoogleGMark className="h-6 w-6 shrink-0" />
        <span className="text-[13px] font-medium tracking-tight text-navy/80">Google</span>
      </span>
      <div
        className="inline-flex items-center gap-px rounded-md bg-white/90 px-1.5 py-0.5 shadow-sm ring-1 ring-black/[0.04]"
        role="img"
        aria-label={`${score} out of 5 stars`}
      >
        {Array.from({ length: 5 }).map((_, i) => (
          <GoogleStyleStar key={i} className={`${starClassName} shrink-0`} />
        ))}
      </div>
      {showScore ? (
        <span className="text-sm font-semibold tabular-nums text-navy">{score}</span>
      ) : null}
    </div>
  );
}
